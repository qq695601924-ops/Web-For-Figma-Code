...
description: 将AI生成的静态Vue组件代码改造为可交互的生产级代码。当用户提到"静态改交互"、"AI生成的代码需要改造"、"footer/header/nav加交互"、"提取公共变量"、"加路由跳转"、"移动端适配"等场景时触发。
---
...
在PC端代码的最外层元素上追加 `md:block hidden`（不要额外包裹新div，避免破坏内部定位），移动端版本作为同级元素紧跟其后，带 `md:hidden block`。
```vue
<!-- PC端：在原有最外层元素上加 md:block hidden -->
<div class="原有class md:block hidden">
  <!-- PC端原内容不动 -->
</div>
...
移动端内容生成规则：
- 去掉所有可跳转内容（链接、按钮、下载入口），只保留纯展示文本
- 将剩余内容按阅读语义分块（如：品牌信息归一块，联系时间归一块，版权归一块）
- 块间距统一 `mb-40`
- 字体统一 `text-60 leading-[1.5]`
- 背景色、字色沿用PC端
